...
export default function AlertsScreenshot() {
  ...
  return (
    <svg viewBox="0 0 256 480" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      <text x="128" y="22" textAnchor="middle" fill="white" fontSize="11" fontWeight="700" fontFamily="Inter, sans-serif">{t('screenshotMockups.alertsScreenshot.headerTitle')}</text>
      <text x="128" y="38" textAnchor="middle" fill="rgba(255,255,255,0.7)" fontSize="8" fontFamily="Inter, sans-serif">{t('screenshotMockups.alertsScreenshot.headerSubtitle')}</text>
      ...
      {alerts.map((alert, i) => (
        <g key={i}>
          ...
          {/* Text */}
          <text x="64" y={93 + i * 100} fill="#1E293B" fontSize="9" fontWeight="700" fontFamily="Inter, sans-serif">{t('screenshotMockups.alertsScreenshot.tappedCard', { child: alert.child })}</text>
          ...
          {!alert.acked ? (
            <g>
              ...
              <text x="199" y={138 + i * 100} textAnchor="middle" fill="white" fontSize="7" fontWeight="600" fontFamily="Inter, sans-serif">{t('screenshotMockups.alertsScreenshot.acknowledgeButton')}</text>
            </g>
          ) : (
            <g>
              <rect x="176" y={128 + i * 100} width="60" height="16" rx="8" fill="#10B981" opacity="0.12" />
              <text x="206" y={140 + i * 100} textAnchor="middle" fill="#10B981" fontSize="7" fontWeight="600" fontFamily="Inter, sans-serif">{t('screenshotMockups.alertsScreenshot.acknowledgedBadge')}</text>
            </g>
          )}
        </g>
      ))}
      ...
      <text x="42" y="474" textAnchor="middle" fill="#94A3B8" fontSize="8" fontFamily="Inter, sans-serif">{t('screenshotMockups.alertsScreenshot.navHome')}</text>
      <text x="128" y="474" textAnchor="middle" fill="#7C3AED" fontSize="8" fontWeight="700" fontFamily="Inter, sans-serif">{t('screenshotMockups.alertsScreenshot.navAlerts')}</text>
      <text x="214" y="474" textAnchor="middle" fill="#94A3B8" fontSize="8" fontFamily="Inter, sans-serif">{t('screenshotMockups.alertsScreenshot.navSettings')}</text>
    </svg>
  )
}
