...
export default function DashboardScreenshot() {
  ...
  return (
    <svg viewBox="0 0 480 320" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      ...
      <text x={mx(80)} y="27" fill="#1E293B" fontSize="12" fontWeight="700" fontFamily="Inter, sans-serif">{t('screenshotMockups.dashboardScreenshot.headerTitle')}</text>
      ...
      {[
        ...
        <g key={i}>
          ...
          <text x={mx(92 + i * 132)} y="82" fill="#1E293B" fontSize="14" fontWeight="800" fontFamily="Inter, sans-serif">{stat.label}</text>
          <text x={mx(92 + i * 132)} y="98" fill="#94A3B8" fontSize="8" fontFamily="Inter, sans-serif">{stat.sub}</text>
        </g>
      ))}

      {/* Children list */}
      <text x={mx(76)} y="138" fill="#1E293B" fontSize="10" fontWeight="700" fontFamily="Inter, sans-serif">{t('screenshotMockups.dashboardScreenshot.yourChildren')}</text>
      {[
        ...
        <g key={i}>
          ...
          <text x={mx(132)} y={171 + i * 68} fill="#1E293B" fontSize="11" fontWeight="600" fontFamily="Inter, sans-serif">{child.name}</text>
          <text x={mx(132)} y={185 + i * 68} fill="#94A3B8" fontSize="8" fontFamily="Inter, sans-serif">{child.age} · {child.events}</text>
      ...
      {/* Recent alerts panel */}
      <text x={mx(76)} y="292" fill="#1E293B" fontSize="10" fontWeight="700" fontFamily="Inter, sans-serif">{t('screenshotMockups.dashboardScreenshot.recentAlerts')}</text>
      {[
        ...
        <g key={i}>
          ...
          <text x={mx(86 + i * 200)} y="311" fill="#334155" fontSize="7" fontFamily="Inter, sans-serif">{alert.msg}</text>
