...
export default function PrivacyPage() {
  ...
  const summaryItems = t('privacyPage.summary.items', { returnObjects: true }) as string[]
    ...
    <>
      ...
      <section className="bg-gradient-to-br from-violet-950 via-slate-900 to-slate-900 pt-32 pb-16">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl font-black text-white mb-4">{t('privacyPage.hero.title')}</h1>
          <p className="text-slate-400 text-lg">{t('privacyPage.hero.effectiveDate')}</p>
          <p className="mt-4 text-slate-300 leading-relaxed max-w-xl mx-auto">
            {t('privacyPage.hero.intro')}
          </p>
        </div>
      </section>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-violet-50 border border-violet-100 rounded-2xl p-6 mb-12">
            <h2 className="text-base font-black text-violet-900 mb-3">{t('privacyPage.summary.title')}</h2>
          ...
          <div className="space-y-12">
            {sectionIds.map((id, i) => {
              const title = t(`privacyPage.sections.${id}.title`)
              const content = t(`privacyPage.sections.${id}.content`, { returnObjects: true }) as ContentItem[]
              return (
                <div key={id}>
